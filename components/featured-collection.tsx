...
/**
 * Editorial asymmetric layout: one large featured piece beside two smaller ones, rather than a
 * uniform card grid — meant to read like a jewelry campaign spread. Falls back to a simple grid
 * if fewer than 3 products are available.
 */
...
function EditorialTile({ product, large = false }: { product: ProductWithRelations; large?: boolean }) {
  ...
  return (
    <Link
      ...
      className={`group relative block overflow-hidden border border-border bg-surface transition-colors duration-500 hover:border-accent ${
    ...
    >
      ...
      {image ? (
        <Image
          ...
          className={`relative h-full w-full object-contain transition-transform duration-700 ease-[var(--ease-luxury)] group-hover:scale-[1.05] ${large ? "p-16" : "p-10"}`}
        />
      ) : (
